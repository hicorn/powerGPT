...
from .config import (
    ...
    InferenceConfig, BenchmarkConfig, add_config_args
)
from .model import GPT
from .trainer import Trainer
from .export import Exporter
from .benchmark import Benchmarker
from .utils import set_seed, get_device_info, is_main_process, print_model_summary
...
class PowerGPTCLI:
    ...
    def load_config(self) -> 'PowerGPTCLI':
        ...
        self.config.update_from_args(vars(self.args))
    ...
    def train(self) -> None:
        ...
        trainer = Trainer(self.config, local_rank=self.args.local_rank if hasattr(self.args, 'local_rank') else -1)
        trainer.train(resume_from=self.args.resume if hasattr(self.args, 'resume') else None)
...
Examples:
  # Train a model
  powergpt train --config configs/t4_124m.yaml
  
  # Generate text
  powergpt generate --checkpoint checkpoints/best_model.pt --prompt "Hello, world!"
  
  # Interactive chat
  powergpt chat --checkpoint checkpoints/best_model.pt
  
  # Run benchmarks
  powergpt benchmark --checkpoint checkpoints/best_model.pt --batch_sizes 1,2,4,8
  
  # Export model
  powergpt export --checkpoint checkpoints/best_model.pt --export_format onnx
        """
    )
