"""
Optimizers and learning rate schedulers for PowerGPT.

Supports:
- AdamW (with fused option for T4)
...
class AdamW(Optimizer):
    ...
    Supports fused kernel for faster execution on CUDA.
    """
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            if group['fused']:
                # Fused kernel (requires contiguous memory)
                ...
                # Simplified - actual fused AdamW would use torch._fused_adamw
                # For brevity, we use standard implementation when fused not fully available
...
class Lion(Optimizer):
    """
    Lion optimizer from Google Research (2023).
    Sign-based optimizer that can be more memory efficient.
...
def create_scheduler(optimizer: Optimizer, scheduler_type: str,
    ...
    Args:
        ...
        scheduler_type: 'cosine', 'cosine_restarts', 'linear', 'constant'
        lr_max: maximum learning rate
        lr_min: minimum learning rate (end of decay)
        warmup_steps: number of warmup steps
        total_steps: total training steps
        restart_interval: steps between restarts (for cosine_restarts)
    
    Returns:
        scheduler object with .step() method
    """
...
# -----------------------------------------------------------------------------
# Learning rate schedule function (standalone)
# -----------------------------------------------------------------------------

def get_lr_schedule(iter_num: int, training_config) -> float:
    """
    Compute learning rate for current iteration using cosine decay with warmup.
    Compatible with TrainingConfig from config.py.
    """
...
def create_optimizer(model: torch.nn.Module, training_config) -> Optimizer:
    """
    Factory function to create optimizer with proper parameter grouping.
    Separates parameters that should have weight decay from those that shouldn't.
    
    Args:
        model: PyTorch model
        training_config: TrainingConfig instance with optimizer parameters
    ...
    for name, param in model.named_parameters():
        if not param.requires_grad:
            continue
        # Don't apply weight decay to bias, LayerNorm/RMSNorm weights
    ...
    if opt_type == 'adamw':
        # Use fused AdamW if available (faster on T4)
        try:
...
def get_param_groups(model: torch.nn.Module, weight_decay: float) -> List[Dict]:
    """
    Create parameter groups for optimizer with custom weight decay.
    """
    decay_params = []
    no_decay_params = []
